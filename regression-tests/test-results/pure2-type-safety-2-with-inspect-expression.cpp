...
auto test_generic(auto const& x, auto const& msg) -> void{
    std::cout 
        ...
        << [&] () -> std::string { auto&& __expr = x;
            if (cpp2::is<int>(__expr)) { if constexpr( requires{std::to_string(cpp2::as<cpp2::failure_policy::throws, int>(x));} ) if constexpr( std::is_convertible_v<CPP2_TYPEOF((std::to_string(cpp2::as<cpp2::failure_policy::throws, int>(x)))),std::string> ) return std::to_string(cpp2::as<cpp2::failure_policy::throws, int>(x)); else return std::string{}; else return std::string{}; }
